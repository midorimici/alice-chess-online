...
io.on('connection', (socket: customSocket) => {
  socket.on(
     ...
     * @param info 入室者のデータ
     */
    (info: { private: boolean; roomId: string; role: 'play' | 'watch'; name: string }) => {
      socket.info = info;
      // public のとき既存の部屋を探し、なかったら新たに生成
      const pubRooms = Array.from(rooms.keys()).filter((k: string) => k[0] === ' ');
        ...
        if (room) {
          ...
          if (room.state === 'waiting') {
            ...
            if (room.winner == null) {
              ...
              socket.emit('tell winner', room.players.map((e) => e.name)[room.winner]);
            }
          }
          // 観戦者数を更新
          room.watchersNum++;
  ...
  socket.on(
     ...
     */
    (boardId: 0 | 1, from: [number, number], to: [number, number], promoteTo?: PieceName) => {
      ...
      // en passant
      if (game.enPassantReq(from, to, pieceName, boardId, boardId, newBoard)) {
        // ポーンを除去する
      ...
      // 勝者を通知する
      if (room.winner != null) {
        io.to(roomId).emit('tell winner', players.map((e) => e.name)[room.winner]);
      }
    }
  );

  socket.on('chat message', (msg: string) => {
    io.to(socket.info.roomId).emit(
      'chat message',
      msg,
      socket.info.role === 'play',
      socket.info.name
    );
  });
